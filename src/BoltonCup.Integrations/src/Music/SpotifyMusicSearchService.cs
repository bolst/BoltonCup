...
namespace BoltonCup.Integrations.Music;

public sealed class SpotifyMusicSearchService : IMusicSearchService
{
    const string TokenCacheKey = "spotify:token";
    ...
    const string PlaylistEndpoint = "https://api.spotify.com/v1/playlists";

    static readonly SemaphoreSlim _tokenLock = new(1, 1);

    readonly HttpClient _httpClient;
    readonly SpotifySettings _settings;
    ...
    async Task<string> FetchAndCacheTokenAsync(CancellationToken cancellationToken)
    {
    ...
    sealed record SearchResponse([property: JsonPropertyName("tracks")] TrackPage? Tracks);

    sealed record TrackPage([property: JsonPropertyName("items")] List<TrackItem> Items);

    sealed record TrackItem(
        [property: JsonPropertyName("id")] string Id,
        [property: JsonPropertyName("name")] string Name,
        [property: JsonPropertyName("artists")] List<Artist> Artists,
        [property: JsonPropertyName("album")] Album? Album);

    sealed record PlaylistTracksPage(
        [property: JsonPropertyName("items")] List<PlaylistItem>? Items,
        [property: JsonPropertyName("next")] string? Next);

    sealed record PlaylistItem([property: JsonPropertyName("track")] TrackItem? Track);

    sealed record Artist([property: JsonPropertyName("name")] string Name);

    sealed record Album([property: JsonPropertyName("images")] List<Image> Images);

    sealed record Image([property: JsonPropertyName("url")] string Url);
}