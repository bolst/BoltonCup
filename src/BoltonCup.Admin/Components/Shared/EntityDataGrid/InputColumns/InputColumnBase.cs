    ...
    where T : class
{
    ...
    Expression<Func<T, TProperty>>? _lastAssignedProperty;
    Func<T, TProperty>? _compiledExpression;
    ...
    {
        if (memberExpression.Expression is not MemberExpression { Member: PropertyInfo propertyInfo } subMemberExpress)
        {
            return item;
        }
