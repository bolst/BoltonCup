...
namespace BoltonCup.Shared
{
    public static class ClaimsPrincipalExtensions
    {
        ...
        public static IEnumerable<int> GetTeamGmIds(this ClaimsPrincipal principal) => principal
                .FindAll(claim => claim.Type == BoltonCupClaimTypes.TeamGm)
                .Select(claim => int.TryParse(claim.Value, out var n) ? n : (int?)null)
                .Where(n => n.HasValue)
                .Select(n => n!.Value);

        public static IEnumerable<int> GetTournamentGmIds(this ClaimsPrincipal principal) => principal
                .FindAll(claim => claim.Type == BoltonCupClaimTypes.TournamentGm)
                .Select(claim => int.TryParse(claim.Value, out var n) ? n : (int?)null)
                .Where(n => n.HasValue)
                .Select(n => n!.Value);

        public static bool IsGmForTeam(this ClaimsPrincipal principal, int teamId) => principal.HasClaim(claim => claim.Type == BoltonCupClaimTypes.TeamGm && claim.Value == teamId.ToString());

        public static bool IsGmForTournament(this ClaimsPrincipal principal, int tournamentId) => principal.HasClaim(claim => claim.Type == BoltonCupClaimTypes.TournamentGm && claim.Value == tournamentId.ToString());
    }
}