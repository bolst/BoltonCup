...
public class InfoGuideRepository(BoltonCupDbContext _context) : IInfoGuideRepository
{
    ...
    public async Task<InfoGuide?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) => await _context.InfoGuides
            .AsNoTracking()
            .Include(e => e.Tournament)
            .OrderByDescending(e => e.LastModified)
            .ThenByDescending(e => e.CreatedAt)
    ...
    public async Task<InfoGuide?> GetByTournamentIdAsync(int tournamentId, CancellationToken cancellationToken = default) => await _context.InfoGuides
            .AsNoTracking()
            .Include(e => e.Tournament)
