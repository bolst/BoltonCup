...
SELECT
	...
	replace(
		goal_song_url,
		'https://iiedqecnfyojvubvugmy.supabase.co/storage/v1/object/public/goal-horns//',
		''
	) AS goal_song_url_key,
	id,
	goal_song_key
FROM
	core.teams
...
for row in df.itertuples(index=False):
    source_key = row.goal_song_url_key
    dest_key = row.goal_song_key
    
    # Skip rows where either key might be null/missing in the database
    if pd.isna(source_key) or pd.isna(dest_key):
        print(f"Skipping row for {row.name}: Missing source or destination key.")
        continue
    ...
    try:
        ...
        print(f"Successfully transferred {row.name}.")
        
    except ClientError as e:
        print(f"Error transferring {source_key} for user {row.name}: {e}")
    except Exception as e:
        print(f"An unexpected error occurred for user {row.name}: {e}")
